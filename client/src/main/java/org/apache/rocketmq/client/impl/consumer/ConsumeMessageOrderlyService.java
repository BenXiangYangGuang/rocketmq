...
public class ConsumeMessageOrderlyService implements ConsumeMessageService {
    ...
    // 消息队列锁容器，严格保证一个消息只有一个线程消费，通过队列锁来实现，一个队列一个锁，获得锁才能进行消息消费。
        ...
        if (context.isAutoCommit()) {
            switch (status) {
                ...
                case SUSPEND_CURRENT_QUEUE_A_MOMENT:
                    ...
                    // 检测消费次数
                    // 如果消息成功进入到DLQ队列，checkReconsumeTimes返回false ，该批消息将直接调用ProcessQueue#commit表示
                    // 消息消费成功，如果这批消息中有任意一条消息的重试次数小于允许的最大重试次数，将返回true，执行消息重试。
                    if (checkReconsumeTimes(msgs)) {
                        // 消息消费重试，先将该批消息重新放入到ProcessQueue的msgTreeMap，然后清除
                        // consumingMsgOrderlyTreeMap 默认延迟1s再加入到消费队列中，并结束此次消息消费。
    ...
    // 最大消费次数Integer.MAX_VALUE，消息重试次数为16；那边有最大次数限定处理
    ...
    // ConsumeRequest的run()方法封装了具体消息消费逻辑
